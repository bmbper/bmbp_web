		...
		<>
			<div className="bm-tree">
				<div className="bm-tree-header">
					<div className="bm-tree-header-title">参数设置</div>
...
const generatorTreeNodes = (treeData: BmbpDict[]) => {
	return treeData.map((item) => {
		const { varsChildren, varsCode, varsName } = item;
		return (
			<Tree.Node
				key={varsCode}
				title={varsName}
				dataRef={item}
			>
				{varsChildren ? generatorTreeNodes(item.varsChildren) : null}
			</Tree.Node>
		);
	});
};
